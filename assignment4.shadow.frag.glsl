...
{
    vec3 shadow_coords = shadow_coord.xyz / shadow_coord.w;
    const float bias = 0.0005; // constant bias

    // Poisson Sampling vector for anti-aliasing
    ...
    // Compare closest depth from light's POV and current depth at fragment 
    // --> if current depth greater, fragment is in shadow
    // Include poisson sampling
    for (int i=0; i< 16; i++){
        if (texture(shadow_tex, shadow_coords.xy + poissonDisk[i]/700.0).r  <  shadow_coords.z - bias) {
            visible = true;
        }
    }
